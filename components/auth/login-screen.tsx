...
import { LoginForm, type LoginStep } from "./login-form";
...
const OTP_COPY: (typeof COPY)[AuthMode] = { title: "Verify your login", subtitle: "We've sent a 6-digit verification code to your registered email." };
...
export function LoginScreen({ hasSession = true, mode = "login" }: { hasSession?: boolean; mode?: AuthMode }) {
  ...
  const copy = step === "otp" ? OTP_COPY : COPY[mode];
      ...
      <section className="flex items-center justify-center px-4 py-10 sm:px-8">
        <div className="w-full max-w-[34rem]">
          ...
          <div className="rounded-3xl border border-line bg-panel p-6 shadow-[0_20px_50px_-24px_rgb(49_46_129/0.25)] sm:p-10">
            {step === "credentials" && <AuthModeSwitch mode={mode} next={params.get("next")} />}
            ...
            {mode === "signup" ? <SignupForm onSuccess={handleSuccess} /> : <LoginForm onSuccess={handleSuccess} onStepChange={setStep} />}
          </div>
        </div>
      </section>
    </div>
