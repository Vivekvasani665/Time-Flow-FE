  ...
  return (
    <form onSubmit={onSubmit} noValidate className="space-y-5">
      <div className="space-y-3">
        ...
        {bothReached &&
          (challenge.sameCodeOnAllChannels === false ? (
            <p className="text-xs text-ink-mute">The email and the SMS each have their own code — either one works.</p>
          ) : (
            <p className="text-xs text-ink-mute">It&apos;s the same code in both — use whichever arrives first.</p>
          ))}
      </div>
